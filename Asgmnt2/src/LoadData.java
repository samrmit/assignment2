			...
			try {
				...
				if (con != null) {
					...
					con.prepareStatement(
							"create table persons (name varchar(100) not null, image varchar(100),status varchar(100),sex varchar(1), age integer, state varchar(3));")
							.execute();

					// inserting initial values to table to demonstrate retrieve from DB
					con.prepareStatement("insert into persons (name, image, status, sex, age, state)"
							+ "values ('Alice', 'alice.jpg','Accountant at RMIT','F',44,'VIC');").execute();
					con.prepareStatement("insert into persons (name, image, status, sex, age, state)"
							+ "values ('James', 'jms.png','Lawyer','M',48,'VIC');").execute();
					con.prepareStatement("insert into persons (name, image, status, sex, age, state)"
							+ "values ('Mike', 'mk.jpg','Student','M',10,'VIC');").execute();

					con.prepareStatement("insert into persons (name, image, status, sex, age, state)"
							+ "values ('Ronald', 'ron.gif','Carpenter','M',28,'NSW');").execute();

					con.prepareStatement("insert into persons (name, image, status, sex, age, state)"
							+ "values ('Ben', 'ben.png','School Teacher','M',36,'WA');").execute();
					con.prepareStatement("insert into persons (name, image, status, sex, age, state)"
							+ "values ('Marie', '','','F',34,'WA');").execute();
					con.prepareStatement("insert into persons (name, image, status, sex, age, state)"
							+ "values ('Steve', 'st.jpg','','M',12,'WA');").execute();

					con.prepareStatement("insert into persons (name, image, status, sex, age, state)"
							+ "values ('Sam', 'jsm.jpg','','F',44,'VIC');").execute();
					con.prepareStatement("insert into persons (name, image, status, sex, age, state)"
							+ "values ('Anne', 'an.jpg','','F',37,'VIC');").execute();
					con.prepareStatement("insert into persons (name, image, status, sex, age, state)"
							+ "values ('Marcus', '','','M',2,'VIC');").execute();

					con.commit();

					rs = con.prepareStatement("select name, image, status, sex, age, state from persons;")
							.executeQuery();

					int i = 0;
					while (rs.next()) {
						System.out.println(rs.getString("name"));
						String[] ar = { rs.getString("name"), rs.getString("image"), rs.getString("status"),
								rs.getString("sex"), rs.getString("age"), rs.getString("state") };
						peopleArray[i] = ar;
						// String[] ar = array();
						peopleList.add(ar);
						i++;
